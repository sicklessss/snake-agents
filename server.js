...
app.post('/api/bot/edit-token', rateLimit({ windowMs: 60_000, max: 20 }), async (req, res) => {
    try {
        ...
        const message = `Snake Agents Edit: ${botId} at ${ts}`;
        let recovered;
        try {
...
app.post('/api/bot/claim', async (req, res) => {
    ...
    try {
        const message = `Claim Snake Agents Bot\nName: ${name}\nAddress: ${address}\nTimestamp: ${timestamp}`;
...
app.post('/api/score/claim-register', async (req, res) => {
    ...
    const message = `SnakeAgents ClaimRegister\nAddress: ${address}\nTimestamp: ${timestamp}`;
    const isValid = await verifyWalletSignature(address, message, signature);
    if (!isValid) {
        return res.status(401).json({ error: 'auth_invalid', message: 'Invalid signature' });
    }
...
async function requireSignatureAuth(req, res, next) {
    ...
    // Verify signature
    const message = `SnakeAgents Referral Auth\nAddress: ${address}\nTimestamp: ${timestamp}`;
    const isValid = await verifyWalletSignature(address, message, signature);
    if (!isValid) {
        return res.status(401).json({ error: 'auth_invalid', message: 'Invalid signature' });
    }
